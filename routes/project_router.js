...
router.get('/', (req, res) => {
    Project
        ...
        .query(function (qb) {
            qb
                .where(`${req.query.searchBy || 'projects.name'}`, 'LIKE', `%${req.query.search || ''}%`)
                .orderBy(req.query.orderBy || 'name', req.query.order || 'asc')
        })
        .fetchPage({
            page: req.query.page || 1,
            pageSize: (req.query.pageSize || 10),
            columns: ['projects.id', 'projects.name', 'description', 'is_active']
        })
        .then(projects => {
            ...
            withRelated: [
                {
                    'components': function (qb) {
                        qb
                            .column('id', 'name', 'price', 'description', 'lead_time', 'min_order_quantity', 'supplier_id')
